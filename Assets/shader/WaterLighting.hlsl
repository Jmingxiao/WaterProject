...
half3 FresnelTerm(half hv, half3 F0)
{
    return F0+(1-F0)*pow((1-hv),5.0);//fresnel TODO - find a better place
}

half DisneyDiffuse(half nv,half nl,half lh,half proughness){
    half Fd90 = 0.5 + 2.0*proughness*lh*lh;
    half lightscatter = (1.0+(Fd90-1.0)*pow(1.0-nl,5.0));
    half viewscatter = (1.0+(Fd90-1.0)*pow(1.0-nv,5.0));
    return lightscatter*viewscatter;
}

float3 fresnelLerp(float cosa, float3 F0, float3 F90)
{
    half t = pow(1-cosa,5.0);
	return lerp(F0,F90,t);
}

float subsurfacescattering( float3 v, float3 l, float3 n,float distortion, float power, float scale){
